...
$nombre = $precio = $zona = $tipo_vivienda = $tipo_contrato = $direccion = $imagen = "";
$message = "";
...
if($_POST){
    ...
    // Crear la propiedad si no hay errores
    if(empty($message)) {
        $result = $property->create();
        
        if($result === true){
            header("Location: index.php?message=Propiedad creada exitosamente");
            exit();
        } elseif($result === "duplicate") {
            $message = "Error: Ya existe una propiedad con el mismo nombre o dirección.";
            // Eliminar la imagen subida si hubo error de duplicado
...
<body>
    <div class="container">
        <h1 class="title">Registro de Propiedades</h1>
